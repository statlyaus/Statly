...
exports.backfillOwnershipPercent =
  exports.processWaiverExpirations =
  exports.onPlayerOwnershipWrite =
  exports.onUserWatchlistUpdate =
  exports.onTeamRosterUpdate =
  exports.processWaivers =
  exports.onTradeUpdate =
  exports.onDraftPickMade =
  exports.processDraftPicks =
    void 0;
...
const DRAFT_WORKER_MEMORY =
  process.env.DRAFT_WORKER_MEMORY || process.env.FUNCTIONS_MEMORY || '1GB'; // '256MB' | '512MB' | '1GB' | '2GB'
...
exports.onDraftPickMade = functions
  ...
  .onWrite(async (change, context) => {
    ...
    // Process only when playerId transitions from empty to a value
    if (
      !(pickData === null || pickData === void 0 ? void 0 : pickData.playerId) ||
      (beforeData === null || beforeData === void 0 ? void 0 : beforeData.playerId) ===
        pickData.playerId
    ) {
    ...
    functions.logger.info(
      `Processing league-specific pick: ${pickData.playerId} for league ${leagueId}`,
      { leagueId, pickId, playerId: pickData.playerId }
    );
    try {
...
exports.onUserWatchlistUpdate = functions
  ...
  .onUpdate(async (change, context) => {
    ...
    const afterData = (_b = change.after.data()) !== null && _b !== void 0 ? _b : {};
    const oldWatchlist =
      ((_c = beforeData.draftPreferences) === null || _c === void 0 ? void 0 : _c.watchlist) || [];
    const newWatchlist =
      ((_d = afterData.draftPreferences) === null || _d === void 0 ? void 0 : _d.watchlist) || [];
...
async function processLeagueDraft(leagueId) {
  ...
  const now = Date.now();
  let pickMillis =
    ((_a = currentPick.pickTime) === null || _a === void 0 ? void 0 : _a.toMillis()) || now;
  ...
  if (timeExpired || currentPick.isAutoPick) {
    functions.logger.info(
      `Processing auto-draft for league ${leagueId}, pick ${currentPick.pickNumber}`
    );
...
async function getAvailablePlayersByStrategy(leagueId, strategy, priorityPositions, round) {
  ...
  return playerDocs.filter((d) => d.exists).map((d) => Object.assign({ id: d.id }, d.data()));
}
async function isPlayerAvailable(leagueId, playerId) {
  ...
  const data = doc.data();
  return (
    ((_b = data === null || data === void 0 ? void 0 : data.leagueAvailability) === null ||
    _b === void 0
      ? void 0
      : _b[leagueId]) !== false
  );
...
async function executeAutoDraftPick(leagueId, pick) {
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (
        ((_a =
          playerData === null || playerData === void 0 ? void 0 : playerData.leagueAvailability) ===
          null || _a === void 0
          ? void 0
          : _a[leagueId]) === false
      ) {
      ...
      tx.set(
        indexRef,
        { available: false, updatedAt: firestore_1.Timestamp.now() },
        { merge: true }
      );
    });
    functions.logger.info(
      `Auto-drafted ${selectedPlayer.name} for user ${pick.userId} in league ${leagueId}`
    );
...
async function createNextDraftPick(leagueId, pickDetails) {
  ...
  try {
    ...
    functions.logger.info(
      `Created next pick ${pickDetails.pickNumber} for team ${team.id} in league ${leagueId}`,
      { leagueId, pickNumber: pickDetails.pickNumber }
    );
  } catch (e) {
    if (isAlreadyExistsError(e)) {
      functions.logger.info(
        `Next pick ${pickDetails.pickNumber} already exists for league ${leagueId}`,
        { leagueId, pickNumber: pickDetails.pickNumber }
      );
    } else {
      throw e;
...
async function initializeAvailablePlayerPool(leagueId) {
  ...
  try {
    ...
    const configured = Number(
      (_b =
        (_a = data === null || data === void 0 ? void 0 : data.waiverRules) === null ||
        _a === void 0
          ? void 0
          : _a.waiverPeriodHours) !== null && _b !== void 0
        ? _b
        : data === null || data === void 0
          ? void 0
          : data.waiverPeriodHours
    );
    ...
    snap.docs.forEach((doc) => {
      ...
      batch.set(
        ref,
        {
          ...
          waiverUntil,
          tier:
            (_a = pdata === null || pdata === void 0 ? void 0 : pdata.tier) !== null &&
            _a !== void 0
              ? _a
              : 999,
          averagePoints:
            (_b = pdata === null || pdata === void 0 ? void 0 : pdata.averagePoints) !== null &&
            _b !== void 0
              ? _b
              : 0,
          position:
            (_c = pdata === null || pdata === void 0 ? void 0 : pdata.position) !== null &&
            _c !== void 0
              ? _c
              : 'UNK',
          updatedAt: firestore_1.Timestamp.now(),
    ...
    await indexRef.set(
      {
        available: true,
        tier:
          (_a = pdata === null || pdata === void 0 ? void 0 : pdata.tier) !== null && _a !== void 0
            ? _a
            : 999,
        averagePoints:
          (_b = pdata === null || pdata === void 0 ? void 0 : pdata.averagePoints) !== null &&
...
async function getUserDraftPreferences(leagueId, userId) {
  ...
  const data = doc.data();
  return (
    (data === null || data === void 0 ? void 0 : data.draftPreferences) || {
      watchlist: [],
...
async function selectBestAvailablePlayer(leagueId, preferences, round) {
  ...
  const availablePlayers = await getAvailablePlayersByStrategy(
    leagueId,
    preferences.draftStrategy,
    (_a = preferences.priorityPositions) !== null && _a !== void 0 ? _a : [],
    round
  );
...
async function setActiveDraft(leagueId, active) {
  ...
  if (active) {
    await ref.set(
      { leagueId, active: true, updatedAt: firestore_1.Timestamp.now() },
      { merge: true }
    );
  } else {
...
async function getLeagueTeamCount(leagueId) {
  ...
  try {
    ...
    const numFromSettings =
      (_b = (_a = leagueDoc.data()) === null || _a === void 0 ? void 0 : _a.settings) === null ||
      _b === void 0
        ? void 0
        : _b.numTeams;
    if (
  ...
  .onRun(async () => {
    ...
    try {
      ...
      snap.docs.forEach((doc) => {
        ...
        batch.set(
          doc.ref,
          { status: 'AVAILABLE', updatedAt: firestore_1.Timestamp.now() },
          { merge: true }
        );
      });
      await batch.commit();
...
exports.backfillOwnershipPercent = functions.region(REGION).https.onRequest(async (req, res) => {
  var _a, _b, _c;
  try {
    ...
    const providedSecret =
      (_a = req.headers['x-internal-secret']) !== null && _a !== void 0 ? _a : undefined;
    ...
    if (providedSecret && internalSecret && safeEquals(providedSecret, internalSecret)) {
      ...
      try {
        ...
        if (
          (decoded === null || decoded === void 0 ? void 0 : decoded.admin) === true ||
          ((_c =
            (_b = decoded === null || decoded === void 0 ? void 0 : decoded.roles) === null ||
            _b === void 0
              ? void 0
              : _b.includes) === null || _c === void 0
            ? void 0
            : _c.call(_b, 'admin'))
        ) {
          authorized = true;
    ...
    ownershipSnap.forEach((doc) => {
      const data = doc.data();
      const ownersCount = Array.isArray(data === null || data === void 0 ? void 0 : data.owners)
        ? data.owners.length
        : 1;
    ...
    indexSnap.forEach((doc) => {
      ...
      batch.set(
        doc.ref,
        { available, ownershipPercent, updatedAt: firestore_1.Timestamp.now() },
        { merge: true }
      );
    });
    await batch.commit();
    ...
    res
      .status(500)
      .json({ error: (e === null || e === void 0 ? void 0 : e.message) || 'Internal error' });
  }
});
...
function isAlreadyExistsError(err) {
  ...
  const code =
    (_b =
      (_a = err === null || err === void 0 ? void 0 : err.code) !== null && _a !== void 0
        ? _a
        : err === null || err === void 0
          ? void 0
          : err.status) !== null && _b !== void 0
      ? _b
      : '';
  ...
  return (
    ...
    /ALREADY[-_ ]?EXISTS/i.test(
      String((err === null || err === void 0 ? void 0 : err.message) || '')
    )
  );
}
