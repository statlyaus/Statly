  ...
  try {
    ...
    for (let i = 0; i < entries.length; i += chunkSize) {
      ...
      chunk.forEach(([userId, total]) => {
        ...
        batch.set(
          ref,
          { pendingBidTotal: total, updatedAt: firestore_1.FieldValue.serverTimestamp() },
          { merge: true }
        );
      });
    ...
    for (let i = 0; i < missingUserIds.length; i += chunkSize) {
      ...
      chunk.forEach((userId) => {
        ...
        batch.set(
          ref,
          { pendingBidTotal: 0, updatedAt: firestore_1.FieldValue.serverTimestamp() },
          { merge: true }
        );
      });
