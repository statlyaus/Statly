...
const AGGREGATOR_TIMEOUT_SECONDS = parseInt(process.env.PLAYER_STATS_AGGREGATOR_TIMEOUT_SECONDS || process.env.FUNCTIONS_TIMEOUT_SECONDS || '540', 10);
const WEIGHTS = {
    kicks: 1,
    handballs: 1,
    marks: 3,
...
function calculateTotalValue(s) {
    ...
    let base = perGame.kicks * WEIGHTS.kicks +
...
function getStat(data, key, altKey) {
    var _a;
    const stats = (_a = data.stats) !== null && _a !== void 0 ? _a : {};
    const direct = parseNumber(stats[key]) || parseNumber(data[key]);
    if (direct)
        return direct;
    if (altKey)
        return parseNumber(stats[altKey]) || parseNumber(data[altKey]);
    return 0;
}
...
async function loadPlayers() {
    ...
    while (true) {
        ...
        snap.docs.forEach((doc) => {
            ...
            if (!name)
                return;
            const nameLC = typeof data.nameLC === 'string' ? data.nameLC : name.toLowerCase();
            players.set(nameLC, {
                id: doc.id,
                name,
                nameLC,
...
async function computeAggregates(season) {
    ...
    aggregates.forEach((agg, key) => {
        ...
        const averages = Object.assign(Object.assign({}, totals), { games, kicks: totals.kicks / games, handballs: totals.handballs / games, marks: totals.marks / games, tackles: totals.tackles / games, goals: totals.goals / games, hitouts: totals.hitouts / games, clearances: totals.clearances / games, inside50s: totals.inside50s / games, rebound50s: totals.rebound50s / games, clangers: totals.clangers / games, contestedPossessions: totals.contestedPossessions / games, uncontestedPossessions: totals.uncontestedPossessions / games, freesFor: totals.freesFor / games, freesAgainst: totals.freesAgainst / games, onePercenters: totals.onePercenters / games, goalAssists: totals.goalAssists / games, turnovers: totals.turnovers / games, intercepts: totals.intercepts / games, metresGained: totals.metresGained / games, contestedMarks: totals.contestedMarks / games, effectiveDisposals: totals.effectiveDisposals / games, scoreInvolvements: totals.scoreInvolvements / games, timeOnGroundPct: avgTog, disposalEffPct: avgDe, seasonTotal: totals.seasonTotal, avgFantasyPoints: totals.avgFantasyPoints, lastGameFantasyPoints: totals.lastGameFantasyPoints });
        const totalValue = calculateTotalValue(Object.assign(Object.assign({}, totals), { games, timeOnGroundPct: avgTog, disposalEffPct: avgDe }));
        const playerId = (_a = playerProfile === null || playerProfile === void 0 ? void 0 : playerProfile.id) !== null && _a !== void 0 ? _a : key;
        rows.push({
            id: playerId,
            player_id: playerId,
            player_name: agg.playerName,
            team: (_b = playerProfile === null || playerProfile === void 0 ? void 0 : playerProfile.team) !== null && _b !== void 0 ? _b : agg.team,
            position: (_d = (_c = playerProfile === null || playerProfile === void 0 ? void 0 : playerProfile.position) !== null && _c !== void 0 ? _c : agg.position) !== null && _d !== void 0 ? _d : 'MID',
            season,
            games,
            totalValue,
...
async function writeToFirestore(season, rows) {
    const chunks = [];
    const batchSize = 400;
