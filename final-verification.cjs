...
async function finalVerification() {
  ...
  console.log(`🔢 Unique rounds: ${Array.from(roundCounts.keys()).sort((a, b) => a - b).join(', ')}`);
  console.log(`📈 Round coverage: ${Math.min(...roundCounts.keys())} to ${Math.max(...roundCounts.keys())}`);
  console.log(`🧹 Arrow symbols remaining: ${arrowCount.withArrows} out of ${arrowCount.total} records`);
...
finalVerification().catch(console.error).finally(() => process.exit(0));
