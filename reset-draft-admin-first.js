...
async function resetDraftToAdminFirst() {
  try {
    ...
    const adminMember = draft.league.members.find(m => 
      m.user.displayName?.includes('Admin') || m.user.email?.includes('admin')
    );
    ...
    members.forEach((member, index) => {
      const isAdmin = member.user.displayName?.includes('Admin') || member.user.email?.includes('admin');
