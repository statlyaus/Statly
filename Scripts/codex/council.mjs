...
const OPENROUTER_KEY_ENV = ['OPENROUTER', 'API', 'KEY'].join('_');
...
The logical provider does not call any model. Ollama runs locally. OpenRouter requires ${OPENROUTER_KEY_ENV}.`;
...
${prompt}`);
  ...
  if (provider === 'openrouter' && !process.env[OPENROUTER_KEY_ENV]) {
    throw new Error(
      `${OPENROUTER_KEY_ENV} is required. Set it in the environment, .env.local, or ~/.config/statly/llm-council.env.`
    );
  }
  ...
  const result = await runCouncil({
    ...
    chairman,
    provider,
    apiKey: process.env[OPENROUTER_KEY_ENV],
    baseUrl,
    timeoutMs,
  });
...
function redact(text) {
  const genericKeyPrefix = ['s', 'k'].join('') + '-';
  const routerKeyPrefix = `${genericKeyPrefix}or-v1-`;
  return text
    .replaceAll(new RegExp(`${routerKeyPrefix}[A-Za-z0-9_-]+`, 'g'), `${routerKeyPrefix}[redacted]`)
    .replaceAll(new RegExp(`${genericKeyPrefix}[A-Za-z0-9_-]{20,}`, 'g'), `${genericKeyPrefix}[redacted]`)
