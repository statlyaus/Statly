...
type BuiltSeasonSummary = Awaited<
  ReturnType<typeof buildPlayerSeasonSummaries>
>['summaries'][number];

function toPlayerSeasonStat(summary: BuiltSeasonSummary): PlayerSeasonStat {
  return {
    ...
    lastUpdated: summary.sourceUpdatedAt,
  };
}

async function loadSeasonSummaryMap(season: number): Promise<Map<string, PlayerSeasonStat>> {
  ...
  return new Map(
    summaries.map((summary) => [summary.playerId, toPlayerSeasonStat(summary)] as const)
  );
}
...
async function validateAgainstOnDemand(
  ...
  season: number,
  seasonSummaryMap: Map<string, PlayerSeasonStat>
): Promise<boolean> {
  const precomputed = seasonSummaryMap.get(player.id) ?? null;
  if (!precomputed) {
  ...
  for (const key of sampleKeys) {
    ...
    if (delta > 0.01) {
      diffs.push(
        `${key}: expected=${expected.toFixed(2)}, actual=${actual.toFixed(2)}, delta=${delta.toFixed(3)}`
      );
    }
  }
...
async function main() {
  ...
  const seasonSummaryMaps = new Map<number, Map<string, PlayerSeasonStat>>();
  for (const season of seasons) {
    seasonSummaryMaps.set(season, await loadSeasonSummaryMap(season));
  }
  ...
  if (validateMode) {
    ...
    for (const player of players.slice(0, 10)) {
      for (const season of seasons) {
        const valid = await validateAgainstOnDemand(
          player,
          season,
          seasonSummaryMaps.get(season) ?? new Map()
        );
        if (valid) passed++;
        else failed++;
      }
  ...
  for (const player of players) {
    for (const season of seasons) {
      processed++;
      try {
        const stat = seasonSummaryMaps.get(season)?.get(player.id) ?? null;

        if (!stat) {
          skipped++;
          if (processed % 50 === 0) {
            console.log(
              `[${processed}/${players.length * seasons.length}] Skipped ${player.name} ${season} (no matches)`
            );
          }
          continue;
        }
        ...
        if (processed % 10 === 0) {
          ...
          console.log(
            `[${processed}/${players.length * seasons.length}] ${player.name} ${season}: ${stat.gamesPlayed} games (${rate.toFixed(1)}/s, ~${Math.round(remaining)}s remaining)`
          );
        }
      } catch (error) {
        errors++;
        console.error(
          `❌ Error processing ${player.name} ${season}:`,
