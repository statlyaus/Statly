...
function resolveAuthEmulatorRestKey(): string {
  return (
    process.env.NEXT_PUBLIC_FIREBASE_API_KEY ||
    [process.env.GOOGLE_CLOUD_PROJECT ?? 'statly', 'emulator'].join('-')
  );
}
...
async function assertAuthLogin(email: string, password: string): Promise<void> {
  const response = await assertFetchOk(
    ...
    `${AUTH_EMULATOR_URL}/identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${encodeURIComponent(
      resolveAuthEmulatorRestKey()
    )}`,
    {
      method: 'POST',
