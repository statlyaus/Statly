...
// Set up environment
const base64Json = require('fs')
  .readFileSync('.env.local', 'utf8')
  .match(/FIREBASE_SERVICE_ACCOUNT_JSON_BASE64=(.+)/)?.[1];
    ...
    if (snapshot.size > 0) {
      ...
      snapshot.docs.forEach((doc, index) => {
        ...
        const categories = {
          ...
          effectiveDisposals:
            data.stats?.effective_disposals || data.raw_row?.effective_disposals || 0,
          scoreInvolvements:
            data.stats?.score_involvements || data.raw_row?.score_involvements || 0,
        };

        console.log(
          `   Categories: G:${categories.goals} T:${categories.tackles} I50:${categories.inside50s} INT:${categories.intercepts}`
        );
        console.log(
          `              CM:${categories.contestedMarks} R50:${categories.rebound50s} CP:${categories.contestedPossessions} ED:${categories.effectiveDisposals} SI:${categories.scoreInvolvements}`
        );
      });
