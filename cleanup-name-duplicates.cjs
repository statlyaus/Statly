...
async function cleanupPlayerNameDuplicates() {
  ...
  const duplicateGroups = Array.from(playerGroups.entries()).filter(
    ([name, records]) => records.length > 1 && new Set(records.map((r) => r.originalName)).size > 1
  );
  ...
  for (const [standardizedName, records] of duplicateGroups) {
    ...
    for (const [round, roundRecords] of roundGroups) {
      if (roundRecords.length > 1) {
        ...
        console.log(
          `    ✅ Keeping: ${bestRecord.originalName} (${bestRecord.data.supercoach_score} SC)`
        );
        ...
        for (const record of roundRecords) {
          if (record.docId !== bestRecord.docId) {
            console.log(
              `    🗑️  Deleting: ${record.originalName} (${record.data.supercoach_score} SC)`
            );
