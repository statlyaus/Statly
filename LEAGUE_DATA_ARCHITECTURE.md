...
class SubscriptionManager {
  ...
  private getRequiredCollections(activeTab: string): string[] {
    switch (activeTab) {
      ...
      case 'trades':
        return ['trades', 'rosters'];
      case 'waivers':
        return ['waivers', 'rosters'];
      default:
        return ['members'];
    }
  }
...
describe('LeagueDataService', () => {
  it('should scope all operations to league', async () => {
    ...
    expect(mockUpdate).toHaveBeenCalledWith(expect.objectContaining({ leagueId }));
  });
});
```
...
class FirestoreUsageTracker {
  ...
  trackRead(leagueId?: string) {
    this.reads++;
    if (leagueId) {
      this.leagueActivity.set(leagueId, (this.leagueActivity.get(leagueId) || 0) + 1);
    }
  }
