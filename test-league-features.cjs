...
const SAMPLE_PLAYERS = [
  // Midfielders
  { id: 'marcus_bontempelli', name: 'Marcus Bontempelli', position: 'MID', team: 'Western Bulldogs' },
...
class LeagueAPITester {
  ...
  async apiCall(endpoint, method = 'GET', body = null, headers = {}) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`API call failed: ${response.status} - ${data.error || data.message || 'Unknown error'}`);
      }
  ...
  async testTradeProposals() {
    ...
    try {
      ...
      try {
        const newTrade = await this.apiCall(`/leagues/${this.leagueId}/trades`, 'POST', sampleTrade);
  ...
  async testWaiverClaims() {
    ...
    try {
      ...
      try {
        const newWaiver = await this.apiCall(`/leagues/${this.leagueId}/waivers`, 'POST', sampleWaiver);
  ...
  async testRosterManagement() {
    ...
    try {
      ...
      // Test lineup setting
      const sampleLineup = {
        DEF: [SAMPLE_PLAYERS[14].id, SAMPLE_PLAYERS[15].id, SAMPLE_PLAYERS[16].id, SAMPLE_PLAYERS[17].id, SAMPLE_PLAYERS[18].id, SAMPLE_PLAYERS[19].id],
        MID: [SAMPLE_PLAYERS[0].id, SAMPLE_PLAYERS[1].id, SAMPLE_PLAYERS[2].id, SAMPLE_PLAYERS[3].id, SAMPLE_PLAYERS[4].id, SAMPLE_PLAYERS[5].id, SAMPLE_PLAYERS[6].id, SAMPLE_PLAYERS[7].id],
        RUC: [SAMPLE_PLAYERS[20].id, SAMPLE_PLAYERS[21].id],
        FWD: [SAMPLE_PLAYERS[8].id, SAMPLE_PLAYERS[9].id, SAMPLE_PLAYERS[10].id, SAMPLE_PLAYERS[11].id, SAMPLE_PLAYERS[12].id, SAMPLE_PLAYERS[13].id],
...
async function main() {
  ...
  try {
    ...
    console.log(`Success Rate: ${Math.round((results.summary.testsPassed / results.summary.testsRun) * 100)}%`);
