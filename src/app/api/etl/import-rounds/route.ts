...
export async function POST(request: NextRequest) {
  ...
  const publicationResult = metadataImportParams.dryRun
    ? null
    : {
        refresh: await refreshPlayerReadModels({
          season: rematerializationParams.season,
          rounds: rematerializationParams.rounds,
        }),
        rankings: await publishPlayerRankings({
          season: rematerializationParams.season,
        }),
        rosters: await publishLeagueRosterSummaries({
          season: rematerializationParams.season,
        }),
      };
    ...
    result: {
      ...
      audit: {
        ...
        rematerialization: publicationResult
          ? {
              refreshedPlayerIds: publicationResult.refresh.refreshedPlayerIds,
              refreshedRounds: publicationResult.refresh.refreshedRounds,
              rankingSnapshots: publicationResult.rankings.rankingSnapshots,
              rosterSummaries: publicationResult.rosters.rosterSummaries,
              rankingsDirty: false,
              rostersDirty: publicationResult.rosters.rostersDirty,
              published:
                publicationResult.rankings.published && publicationResult.rosters.published,
            }
          : null,
        verifierCommand,
      },
