...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const pauseCount = 0;
    const totalDuration =
      draftTimes?.startedAt && draftTimes?.completedAt
    ...
    const perCounts = new Map<
      string,
      { picks: number; auto: number; timeSum: number; samples: number }
    >();
    for (let i = 0; i < picksOrdered.length; i++) {
      ...
      if (i > 0) {
        const dt = Math.max(
          0,
          (picksOrdered[i].madeAt.getTime() - picksOrdered[i - 1].madeAt.getTime()) / 1000
        );
        rec.timeSum += dt;
        rec.samples += 1;
      }
