...
export default function RostersPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const owned: RosterPlayer[] =
            (json.data?.roster?.players || []).map(mapPlayer);
          ...
          if (faRes.ok) {
            ...
            const fa: RosterPlayer[] =
              (faJson.data?.players || []).map(mapPlayer);
  ...
  const handleClaim = async (player: RosterPlayer, isWaiver: boolean) => {
    if (pendingClaim[player.id]) return;
    setPendingClaim((prev) => ({ ...prev, [player.id]: true }));

    let prevRoster: RosterPlayer[] = [];
    let prevFA: RosterPlayer[] = [];
    ...
    try {
      ...
      console.log('Player action succeeded');
    } catch (e) {
      console.error('Failed to process player action', e);
      setRosterPlayers(prevRoster);
      setFreeAgents(prevFA);
    } finally {
    ...
    <AppLayout>
      <main className="p-6 space-y-8">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {freeAgents.map((p) => {
              const expiryMs = typeof p.waiverExpiresAt === 'number' ? p.waiverExpiresAt : undefined;
