...
export default function RostersPage() {
  ...
  const handleClaim = async (player: RosterPlayer, isWaiver: boolean) => {
    try {
      ...
      // TODO: refresh roster/freeAgents or update state optimistically
    } catch (e) {
      ...
      <main className="p-6 space-y-8">
        ...
        <section>
          ...
          {rosterPlayers.length === 0 ? (
            <p className="text-center text-gray-500">No players on roster.</p>
          ) : (
            <FixedSizeList
              height={Math.min(400, rosterPlayers.length * 100)}
              itemCount={rosterPlayers.length}
              itemSize={100}
              width="100%"
            >
              {({ index, style }) => {
                const p = rosterPlayers[index];
                return (
        ...
        <section>
          ...
          {freeAgents.length === 0 ? (
            <p className="text-center text-gray-500">No available players.</p>
          ) : (
            <FixedSizeList
              height={Math.min(400, freeAgents.length * 100)}
