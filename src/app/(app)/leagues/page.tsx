...
type LeagueSummary = League & { memberCount?: number };
...
function getAvailableTeamSlots(league: LeagueSummary): number {
  const occupiedTeams =
    typeof league.currentTeams === 'number'
      ? league.currentTeams
      : typeof league.memberCount === 'number'
        ? league.memberCount
        : 0;

  return Math.max((league.maxTeams || 0) - occupiedTeams, 0);
}

export default function LeaguesPage(): JSX.Element {
          ...
          <section className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              { label: 'Active', value: activeCount, icon: ShieldCheck },
              {
                ...
                value: leagues.reduce((sum, league) => sum + getAvailableTeamSlots(league), 0),
                icon: Users,
              },
            ].map((item) => {
