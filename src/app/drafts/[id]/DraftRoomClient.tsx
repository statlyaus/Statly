...
export default function DraftRoomClient({ players, draftData }: DraftRoomClientProps) {
  ...
  const [fantasyScoreRange, setFantasyScoreRange] = useState<[number, number]>([0, 100]);
  ...
  const filteredPlayers = useMemo(() => {
    let filtered = players.filter(player => {
      ...
      if (injuryFilter !== 'ALL') {
        ...
        if (injuryFilter === 'INJURED' && !['questionable', 'injured', 'out'].includes(playerInjuryStatus)) {
          return false;
        }
      }
  ...
  const getDraftState = useCallback(() => {
    ...
    const currentDrafterId = currentTurnIndex < draftOrder.length ? draftOrder[currentTurnIndex] : null;
  ...
  const getTeamCategoryAnalysis = useCallback(() => {
    ...
    const categoryAnalysis = CORE_FANTASY_CATEGORIES.map(category => {
      ...
      return {
        ...
        teamTotal,
        teamAverage,
        leagueAverage: allPlayerAverages.length > 0 ? allPlayerAverages[Math.floor(allPlayerAverages.length * 0.5)] : 0,
  ...
  const getPlayerRecommendations = useCallback((count: number = 5): DraftPlayer[] => {
    if (!recommendationCriteria.considerTeamNeeds) {
      return filteredPlayers
        .filter(player => !recommendationCriteria.avoidInjured || !player.injuryStatus || player.injuryStatus === 'healthy')
        .slice(0, count);
    }

    const teamAnalysis = getTeamCategoryAnalysis();
    const myPicks = draftData.picks.filter(pick => pick.member.id === 'current-user');
    const positionCounts = myPicks.reduce((acc, pick) => {
    ...
    const scoredPlayers = filteredPlayers
      ...
      .map(player => {
        ...
        if (player.stats && teamAnalysis.needsImprovement.length > 0) {
          ...
          teamAnalysis.needsImprovement.forEach(weakCategory => {
            ...
            if (playerCategoryAverage > leagueAverage) {
              ...
              const improvementScore = (playerCategoryAverage / leagueAverage) * weakCategory.weight * 15;
          ...
          teamAnalysis.strongCategories.forEach(strongCategory => {
            ...
            if (playerCategoryAverage > leagueAverage) {
              const maintenanceScore = (playerCategoryAverage / leagueAverage) * strongCategory.weight * 5;
        ...
        const maxForPosition = leagueCustomization.positionLimits[player.position as keyof typeof leagueCustomization.positionLimits] || 8;
  ...
  }, [filteredPlayers, recommendationCriteria, draftData.picks, leagueCustomization, getTeamCategoryAnalysis]);
  ...
  const validatePick = useCallback((player: DraftPlayer): { isValid: boolean; errors: string[] } => {
    const errors: string[] = [];
    ...
    if (currentTime - pickValidation.lastValidationTime < 1000) { // 1 second cooldown
  ...
  useEffect(() => {
    ...
    const currentUserId = 'current-user'; // TODO: Replace with actual user ID from auth
    const isUsersTurn = draftState.currentDrafter?.member.id === currentUserId && !draftState.isComplete;
  ...
  }, [getDraftState, autoPickEnabled, draftData.status, filteredPlayers, setIsMyTurn, setTimeRemaining, leagueCustomization.autoPickTime, handlePlayerSelect]);
  ...
  const PlayerRow = ({ player }: { player: DraftPlayer }) => {
    ...
    return (
      <tr key={player.id} className={`border-b transition-colors ${
        isAlreadyPicked 
        ...
        <td className={`sticky left-0 border-r border-gray-200 z-10 px-4 py-3 ${
          isAlreadyPicked 
            ? 'bg-red-50' 
            : isPlayerValid 
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {connectionState.status !== 'connected' && (
        <div className={`w-full px-4 py-2 text-center text-white ${
          connectionState.status === 'disconnected' ? 'bg-red-600' : 
          connectionState.status === 'reconnecting' ? 'bg-yellow-600' : 'bg-blue-600'
        }`}>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    {draftData.status !== 'COMPLETED' && draftState.currentRound === 1 && draftData.picks.length === 0 && (
                      <button
                        onClick={() => setDraftOrderManagement(prev => ({ ...prev, showOrderModal: true }))}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="font-medium">
                          {isMyTurn ? 'Your Turn!' : `${draftState.currentDrafter.member.displayName}'s Turn`}
                        </span>
                  ...
                  <div className="bg-white/50 rounded-lg p-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      {draftState.nextPicks.map((nextPick) => (
                        <div key={nextPick.pickNumber} className="flex items-center space-x-2 text-sm">
                          <span className="text-indigo-600 font-medium">#{nextPick.pickNumber}</span>
                          <span className="text-gray-700">{nextPick.participant.member.displayName}</span>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-gray-600">
                Focus on <span className="font-medium text-red-600">weak areas</span> • Maintain <span className="font-medium text-green-600">strengths</span>
              </div>
            </div>
              ...
              <div className="flex items-center space-x-4">
                {getTeamCategoryAnalysis().needsImprovement.length > 0 && (
                  <div className="flex items-center text-red-700">
                    ...
                    <span className="ml-1">
                      {getTeamCategoryAnalysis().needsImprovement.slice(0, 2).map(cat => cat.name).join(', ')}
                    </span>
                  </div>
                )}
                {getTeamCategoryAnalysis().strengths.length > 0 && (
                  <div className="flex items-center text-green-700">
                    ...
                    <span className="ml-1">
                      {getTeamCategoryAnalysis().strengths.slice(0, 2).map(cat => cat.name).join(', ')}
                    </span>
                  </div>
                )}
              </div>
        ...
        <div className="w-full px-4 py-2 bg-blue-50 border-b border-blue-200">
          <div className="max-w-7xl mx-auto text-center">
            <div className="flex items-center justify-center space-x-2 text-blue-800">
              ...
              <span className="font-medium">
                Auto-pick in {Math.floor(timeRemaining / 60)}:{(timeRemaining % 60).toString().padStart(2, '0')}
              </span>
              <button 
              ...
              <div key={player.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">{player.position} - {player.club}</div>
                  </div>
                </div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`text-sm font-mono ${
                    liveDraftState.timeRemaining <= 30 ? 'text-red-600 font-bold' : 
                    liveDraftState.timeRemaining <= 60 ? 'text-yellow-600 font-medium' : 'text-gray-700'
                  }`}>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-1">
                  <div className={`w-2 h-2 rounded-full ${
                    ...
                    connectionState.status === 'reconnecting' ? 'bg-yellow-500' : 'bg-red-500'
                  }`}></div>
                ...
                {!liveDraftState.isYourTurn && liveDraftState.picksUntilYourTurn > 0 && (
                  <div className="text-sm text-gray-600">
                    Your turn in {liveDraftState.picksUntilYourTurn} pick{liveDraftState.picksUntilYourTurn !== 1 ? 's' : ''}
                  </div>
                )}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-600">
                {lastPickMade.player.position} • {lastPickMade.player.club} • Pick #{lastPickMade.overall}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-3 sm:space-y-0">
            <div>
              ...
              <p className="text-gray-600 text-sm">
                Pick {liveDraftData.currentPick} of {liveDraftData.totalPicks} | Round {Math.ceil(liveDraftData.currentPick / liveDraftData.participants.length)}
              </p>
              <div className="flex items-center space-x-2 mt-1">
                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                  liveDraftData.status === 'LIVE' ? 'bg-green-100 text-green-800' :
                  liveDraftData.status === 'ACTIVE' ? 'bg-blue-100 text-blue-800' :
                  liveDraftData.status === 'COMPLETED' ? 'bg-gray-100 text-gray-800' :
          ...
          <div className="bg-white rounded-lg border p-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              <div className="col-span-1 sm:col-span-2 md:col-span-1">
                <label htmlFor="search" className="block text-sm font-medium mb-1">Search Players</label>
              ...
              <div>
                <label htmlFor="injuryFilter" className="block text-sm font-medium mb-1">Injury Status</label>
                <select
                  id="injuryFilter"
                  value={injuryFilter}
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            ...
                            setRecommendationCriteria(prev => ({
                              ...prev,
                              prioritizePositions: prev.prioritizePositions.filter(p => p !== position)
                            }));
                          }
                        }}
                        className="mr-2"
              ...
              <div className="md:col-span-2">
                ...
                <div className="grid grid-cols-4 gap-2 text-sm">
                  {POSITIONS.filter(pos => pos !== 'ALL').map(position => {
                    ...
                    const maxForPosition = leagueCustomization.positionLimits[position as keyof typeof leagueCustomization.positionLimits] || 8;
                    return (
                      <div key={position} className="bg-gray-50 p-2 rounded text-center">
                        <div className="font-medium">{position}</div>
                        <div className={`text-xs ${currentCount >= maxForPosition ? 'text-red-600' : 'text-green-600'}`}>
          ...
          {showRecommendations && (
            <div className="bg-white rounded-lg border overflow-hidden">
              <div className="p-4 bg-blue-50 border-b">
                ...
                <p className="text-sm text-blue-600">Based on your team needs and league settings</p>
              </div>
                ...
                <Table className="text-left w-full min-w-max">
                  ...
                  <tbody>
                    {getPlayerRecommendations(10).map((player, index) => (
                      <tr key={player.id} className={`border-b hover:bg-blue-50 ${index < 3 ? 'bg-yellow-50' : ''}`}>
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`px-2 py-1 rounded text-xs font-medium ${
                            (player.recommendationScore || 0) > 100 ? 'bg-green-100 text-green-700' :
                            (player.recommendationScore || 0) > 75 ? 'bg-yellow-100 text-yellow-700' :
                            'bg-gray-100 text-gray-700'
                          }`}>
                        ...
                        <td className="px-2 py-3 text-xs">
                          {(() => {
                            if (!player.stats) return <div className="text-gray-400">No data</div>;
                            ...
                            const categoryImpacts = CORE_FANTASY_CATEGORIES.map(cat => {
                              ...
                              const teamCat = teamAnalysis.categories.find(tc => tc.key === cat.key);
                            ...
                            return (
                              <div className="space-y-1">
                                {categoryImpacts.map(imp => (
                                  <div key={imp.category.key} className={`flex items-center justify-between ${
                        ...
                        <td className="px-4 py-3 text-sm">
                          {recommendationCriteria.prioritizePositions.includes(player.position) && '🎯 Priority Position '}
                          {(() => {
                            const currentCount = draftData.picks
                          ...
                          {(() => {
                            ...
                            const helpfulWeakCategories = teamAnalysis.needsImprovement.filter(weakCat => {
                            ...
                            const reinforcesStrengths = teamAnalysis.strengths.filter(strongCat => {
                ...
                {recentActivity.slice(0, 10).map((activity) => (
                  <div key={activity.id} className="flex items-start space-x-2 text-sm">
                    <span className={`inline-block w-2 h-2 rounded-full mt-1.5 ${
                      activity.type === 'pick' ? 'bg-green-500' :
        ...
        <div className="bg-white rounded-lg border overflow-hidden">
          ...
          <Table className="text-left">
            ...
            <tbody>
              ...
              {liveDraftData.picks.filter(pick => pick.member.id === liveDraftData.participants[0]?.member.id).length === 0 && (
                <tr>
        ...
        {confirmModal.player && (
          <div className="p-6">
            ...
            <div className="mb-6">
              ...
              <div className={`p-4 rounded border-2 ${isMyTurn ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200'}`}>
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  {confirmModal.player.injuryStatus && confirmModal.player.injuryStatus !== 'healthy' && (
                    ...
                    <div className="flex items-center space-x-2">
                      {validation.isValid ? (
                        <>
                          ...
                          <span className="text-green-800 font-medium">Pick validated - Ready to draft!</span>
                        </>
                      ) : (
                        <>
                ...
                return (
                  ...
                  >
                    {pickValidation.isPicking ? (
                      ...
                      isMyTurn ? 'Confirm Pick' : 'Override Pick'
                    ) : (
                      'Cannot Draft'
                    )}
                  </Button>
        ...
        <div className="p-6 max-w-4xl w-full max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="autoPickTime" className="block text-sm font-medium mb-1">Auto-pick Timer (seconds)</label>
                <input
                  id="autoPickTime"
                  type="number"
                  min="30"
                  ...
                  onChange={(e) => setLeagueCustomization(prev => ({
                    ...prev,
              ...
              <div>
                <label htmlFor="draftStyle" className="block text-sm font-medium mb-1">Draft Style</label>
                <select
                  id="draftStyle"
            ...
            <div className="space-y-4">
              ...
              {Object.entries(leagueCustomization.positionLimits).map(([position, limit]) => (
                <div key={position}>
                  <label htmlFor={`limit-${position}`} className="block text-sm font-medium mb-1">{position} Limit</label>
                  <input
                    id={`limit-${position}`}
                    type="number"
                    min="1"
            ...
            <div className="space-y-4 md:col-span-2">
              ...
              <div>
                <label htmlFor="defaultSort" className="block text-sm font-medium mb-1">Default Player Sort</label>
                <select
                  id="defaultSort"
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['goals', 'kicks', 'handballs', 'marks', 'tackles', 'hitouts', 'clearances', 'inside50s', 'intercepts'].map(stat => (
      ...
      <Modal open={draftOrderManagement.showOrderModal} onClose={() => setDraftOrderManagement(prev => ({ ...prev, showOrderModal: false }))}>
        <div className="p-6 max-w-2xl">
          ...
          <div className="space-y-6">
                ...
                <div className="space-y-2 bg-gray-50 p-3 rounded-lg">
                  {(() => {
                    ...
                    return draftState.draftOrder.map((userId, index) => {
                      ...
                      return participant ? (
                        <div key={userId} className="flex items-center space-x-3 p-2 bg-white rounded">
                ...
                <div className="space-y-2 bg-purple-50 p-3 rounded-lg">
                  {draftOrderManagement.isRandomizing ? (
                    ...
                    draftOrderManagement.tempOrder.map((userId, index) => {
                      const participant = draftData.participants.find(p => p.member.id === userId);
                      return participant ? (
                        <div key={userId} className="flex items-center space-x-3 p-2 bg-white rounded shadow-sm">
                          <span className="w-6 h-6 bg-purple-100 text-purple-700 rounded-full flex items-center justify-center text-sm font-medium">
                            {index + 1}
                          </span>
                          <span className="text-sm">{participant.member.displayName}</span>
                          {index === 0 && <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded">First Pick</span>}
                        </div>
                      ) : null;
                    })
                  ) : (
                    <div className="text-center py-8 text-gray-500">
                      <p className="text-sm">Click &ldquo;Randomize&rdquo; to generate a new order</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex justify-end space-x-3">
              <button
                onClick={() => setDraftOrderManagement(prev => ({ ...prev, showOrderModal: false }))}
