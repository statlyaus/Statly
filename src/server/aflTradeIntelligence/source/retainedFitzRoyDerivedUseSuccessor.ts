...
export function createRetainedFitzRoyDerivedUseSuccessor(
  ...
  const approval = z
    ...
    .passthrough()
    .parse(JSON.parse(Buffer.from(input.ownerApprovalBytes).toString('utf8')));
  ...
  const originalOperations = proposed.scope.dimensions.find(
    ({ name }) => name === 'operation'
  )?.values;
  if (
    !Number.isFinite(effective) ||
    original.effectiveAt === null ||
    ...
    candidate.methodUseId !==
      createAflTradeContentAddress('cameron-2020-hpn-method-use', candidate.content) ||
    createHash('sha256').update(input.ownerApprovalBytes).digest('hex') !==
      candidate.content.ownerApprovalSha256 ||
    !approval.ownerAuthorization.approvedUses.includes('valuation and grading') ||
    !approval.ownerAuthorization.approvedUses.includes(
    ...
    capture === undefined ||
    capture.originalGateDecisionId !== originalDecision.decisionId ||
    ...
    canonicalizeAflTradeJson(capture.sourceFields) !==
      canonicalizeAflTradeJson([...capture.sourceFields].sort()) ||
    capture.sourceFields.some(
      (field) =>
        originalRights.content.fields.find((item) => item.sourceField === field)?.uses
    ...
    !originalRights.content.scope.seasonRanges.some(
      (range) => range.from <= 2020 && range.to >= 2020
    ) ||
    original.environment !== 'non_production' ||
    original.gate !== 'gate_0a_permission_to_evaluate' ||
    original.state !== 'approved' ||
    original.proposalId !== originalProposal.proposalId ||
    ...
    original.decisionKey !== proposed.decisionKey ||
    original.version !== proposed.version ||
    proposed.environment !== original.environment ||
    proposed.gate !== original.gate ||
    ...
    canonicalizeAflTradeJson(proposed.affectedArtifacts) !==
      canonicalizeAflTradeJson(original.affectedArtifacts) ||
    !proposed.scope.dimensions.some(
      ({ name, values }) =>
    ...
    !proposed.scope.dimensions.some(
      ({ name, values }) => name === 'season' && values.includes('2020')
    ) ||
    !proposed.scope.dimensions.some(
      ({ name, values }) => name === 'fitzroy_capability' && values.includes(capture.capabilityId)
    ) ||
    !originalRights.content.acquisition.capabilities.some(
      (item) => item.capabilityId === capture.capabilityId
    ) ||
    originalOperations?.includes('derived_feature_creation') ||
    proposed.scope.dimensions.some(
      ({ name }) => name.startsWith('retained_') || name.startsWith('original_')
    ) ||
    proposed.conditions.length !== original.conditionResults.length ||
    proposed.conditions.some(
      (condition) =>
        !original.conditionResults.some(
          (result) =>
  ...
  const rightsContent = {
    ...
    operations: Object.fromEntries(
      AFL_TRADE_SOURCE_OPERATIONS.map((operation) => [
        operation,
        allowedPrivateOperations.has(operation) ? 'allowed' : 'blocked',
      ])
    ),
    fields: originalRights.content.fields.map((field) => ({
      ...
      uses: {
        ...field.uses,
        derived_feature: selectedFields.has(field.sourceField) ? 'allowed' : 'blocked',
  ...
  const scope = {
    ...proposed.scope,
    description:
      'Thirty-day private derived use of one exact retained 2020 capture and method; no new capture or public use.',
    dimensions: [
  ...
  const proposalContent = {
    ...
    scope,
    proposal:
      'Permit only the documented 2020 HPN private derived calculation on this retained capture for thirty days.',
  ...
  const decisionContent = {
    ...
    rationale:
      'Owner-authorized exact private derived use of retained evidence; no external licence is asserted.',
    limitations: [
