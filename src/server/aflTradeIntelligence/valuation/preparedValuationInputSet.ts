...
export const aflTradePrivatePreparedValuationAuthoritySchema = z
  .object({
  ...
  .superRefine((authority, context) => {
    if (authority.playerRunId === authority.pickRunId) {
      context.addIssue({
        code: 'custom',
        message: 'Private prepared authority requires distinct player and pick runs.',
      });
    }
  });
