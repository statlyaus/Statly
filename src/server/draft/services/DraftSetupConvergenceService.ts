...
import { DraftApplicationService } from './DraftApplicationService';
...
export interface DraftSetupConvergenceInput {
  ...
  leagueId: string;
  now?: Date;
  draftApplicationService?: Pick<DraftApplicationService, 'startDraft'>;
}

function isBenignStartRace(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return (
    message.startsWith('bad_request:Draft') ||
    message.startsWith('conflict:Draft state changed') ||
    message.startsWith('conflict:Draft lobby state changed') ||
    message.startsWith('conflict:Draft scheduling changed')
  );
}
...
export async function ensureLeagueDraftSetupConverged(
  ...
  const draftApplicationService = input.draftApplicationService ?? new DraftApplicationService();
  ...
  if (readiness.lifecycle.canStartClock && readiness.draftId) {
    try {
      await draftApplicationService.startDraft({ draftId: readiness.draftId });
    } catch (error) {
  ...
  return getLeagueDraftOperationalReadiness(client, {
    leagueId: input.leagueId,
    now: input.now,
  });
}
