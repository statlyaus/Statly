...
import type { DraftState, DraftParticipant, DraftPick, DraftPlayer, DraftSettings } from '@/types/draft';
...
export function useDraftService(
  draftId: string,
  options: UseDraftServiceOptions = {}
) {
  const {
    enableCaching = true,
  ...
  const draftService: DraftService = useMemo(() => ({
    ...
    makePick: async (playerId: string) => {
      ...
      try {
        const pick = await apiCall<DraftPick>(
          `drafts/${draftId}/pick`,
          {
            method: 'POST',
