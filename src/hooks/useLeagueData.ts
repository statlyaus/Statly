...
export interface UseLeagueDataReturn {
  ...
  submitWaiverClaim: (claim: Omit<LeagueWaiverClaim, 'id' | 'leagueId' | 'createdAt'>) => Promise<string>;
...
export function useLeagueData({
  ...
  const updateMemberPreferences = useCallback(async (updates: Partial<LeagueMember>) => {
    if (!userId) {
  ...
  const proposeTrade = useCallback(async (
    trade: Omit<LeagueTrade, 'id' | 'leagueId' | 'createdAt'>
  ): Promise<string> => {
    try {
  ...
  // Subscription management
  const subscribe = useCallback((collections: string[]) => {
  ...
  // Utility functions
  const getUserTeam = useCallback((targetUserId: string): LeagueRoster | null => {
    return rosters.find(roster => roster.userId === targetUserId) || null;
  }, [rosters]);
  
  const getTeamOwner = useCallback((teamId: string): LeagueMember | null => {
    const roster = rosters.find(r => r.id === teamId);
    if (!roster) return null;
    return members.find(member => member.userId === roster.userId) || null;
  }, [rosters, members]);
  
  const getUserTrades = useCallback((targetUserId: string): LeagueTrade[] => {
    return trades.filter(trade => 
      trade.fromUserId === targetUserId || trade.toUserId === targetUserId
    );
  }, [trades]);
  
  const getUserWaivers = useCallback((targetUserId: string): LeagueWaiverClaim[] => {
    return waiverClaims.filter(claim => claim.userId === targetUserId);
  }, [waiverClaims]);
  
  const getDraftPicksForTeam = useCallback((teamId: string): LeagueDraftPick[] => {
