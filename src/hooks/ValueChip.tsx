...
type Props = { playerId: string; compact?: boolean };

export default function ValueChip({ playerId, compact = false }: Props) {
  const { get, isLoading, error } = useRankings();
  const data = get(String(playerId));

  if (error) return null;                  // or show a tiny error marker
