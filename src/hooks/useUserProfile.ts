...
export function useUserProfile(userId?: string): UseUserProfileReturn {
  ...
  const updateLeagueSettings = useCallback(
    async (leagueId: string, settings: Partial<LeagueSpecificSettings>) => {
      if (!currentUserId) {
        throw new Error('User not authenticated');
      }
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to update league settings';
        setError(errorMessage);
   ...
   */
  const getDraftWatchlists = useCallback(
    async (leagueId: string): Promise<UserWatchlist[]> => {
      if (!currentUserId) {
        throw new Error('User not authenticated');
      }
   ...
   */
  const getNextDraftPlayer = useCallback(
    async (leagueId: string, excludePlayerIds: string[] = []): Promise<string | null> => {
      if (!currentUserId) {
        throw new Error('User not authenticated');
      }
  ...
  const getLeague = useCallback(
    (leagueId: string): LeagueMembership | null => {
   ...
   */
  const getLeagueSettings = useCallback(
    (leagueId: string): LeagueSpecificSettings | null => {
  ...
  const getWatchlist = useCallback(
    (leagueId?: string): UserWatchlist | null => {
