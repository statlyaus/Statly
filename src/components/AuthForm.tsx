...
const AuthForm = ({ initialMode = 'login', onSuccess, className = '', nextUrl, autoRedirectIfAuthenticated = false, showModeSwitch = true }: AuthFormProps) => {
      ...
      >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <EnvelopeIcon className={`w-5 h-5 transition-colors ${
                    !validation.email.isValid ? 'text-red-400' : 
                    email && validation.email.isValid ? 'text-green-500' : 
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <LockClosedIcon className={`w-5 h-5 transition-colors ${
                    !validation.password.isValid ? 'text-red-400' : 
                    password && validation.password.isValid ? 'text-green-500' : 
                    'text-slate-400 group-focus-within:text-blue-500'
                  }`} />
                </div>
                <input
                ...
                <div className="mt-3">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className={`text-xs font-semibold ${
                      getPasswordStrength(password) < 25 ? 'text-red-600 dark:text-red-400' :
                      getPasswordStrength(password) < 50 ? 'text-yellow-600 dark:text-yellow-400' :
                      getPasswordStrength(password) < 75 ? 'text-blue-600 dark:text-blue-400' : 'text-green-600 dark:text-green-400'
                    }`}>
                  ...
                  <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                    <div 
                      className={`h-full rounded-full transition-all duration-500 ${
                        ...
                        getPasswordStrength(password) < 50 ? 'bg-yellow-500' :
                        getPasswordStrength(password) < 75 ? 'bg-blue-500' : 'bg-green-500'
                      }`}
              ...
              {!validation.password.isValid && (
                <p id="password-error" className="text-sm text-red-600 dark:text-red-400 flex items-center gap-1 mt-2">
                  <ExclamationTriangleIcon className="w-4 h-4" />
                ...
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <LockClosedIcon className={`w-5 h-5 transition-colors ${
                      ...
                      confirmPassword && password === confirmPassword ? 'text-green-500' : 
                      'text-slate-400 group-focus-within:text-blue-500'
                    }`} />
                  </div>
                  <input
