...
function parseReturnToDays(expectedReturn?: string, status?: string): number {
  const base =
    typeof expectedReturn === 'string' ? expectedReturn : typeof status === 'string' ? status : '';
  ...
  const sortedInjuries = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'return':
        arr.sort(
          (a, b) =>
            parseReturnToDays(a.expectedReturn, a.status) -
            parseReturnToDays(b.expectedReturn, b.status)
        );
        break;
      case 'name':
