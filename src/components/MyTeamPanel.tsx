...
const MyTeamPanel = ({ team, players }: MyTeamPanelProps) => {
  ...
  const draftedPlayers = players.filter((p) => (team.players ?? []).map(String).includes(String(p.id)));
          ...
          <ul className="text-sm space-y-1 max-h-[600px] overflow-y-auto">
            {draftedPlayers.map((player) => (
              <li key={player.id} className="border-b py-1">
                <span className="font-medium">{capitalizeWords(player.name)}</span> – {player.team ? capitalizeFirstLetter(player.team) : ""} ({player.position ? capitalizeFirstLetter(player.position) : ""})
              </li>
            ))}
          </ul>
        )}
