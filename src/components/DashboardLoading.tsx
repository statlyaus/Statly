export default function DashboardLoading() {
  return (
    <main
      className="min-h-screen bg-[linear-gradient(180deg,var(--league-surface)_0%,var(--league-page)_42%,var(--league-surface-muted)_100%)]"
      role="main"
      aria-busy="true"
    >
          ...
          <section className="rounded-[2rem] border border-border bg-white/92 p-5 shadow-[0_22px_70px_-45px_rgba(15,23,42,0.35)] sm:p-6">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-3xl space-y-3">
                ...
                <div className="h-10 w-full max-w-[34rem] animate-pulse rounded bg-muted" />
                <div className="h-5 w-full max-w-[40rem] animate-pulse rounded bg-muted" />
              </div>
              <div className="flex flex-wrap gap-2">
                {Array.from({ length: 3 }).map((_, index) => (
                  <div
                    key={index}
                    className="h-9 w-32 animate-pulse rounded-full border border-border bg-muted"
                  />
                ))}
              </div>
            </div>
            ...
            <div className="mt-6 grid gap-3 lg:grid-cols-[minmax(0,1.2fr)_repeat(2,minmax(0,0.9fr))]">
              <div className="rounded-[1.5rem] bg-foreground px-5 py-5">
              ...
              {Array.from({ length: 2 }).map((_, index) => (
                <div
                  key={index}
                  className="rounded-[1.5rem] border border-border bg-muted px-5 py-5"
                >
                  <div className="h-3 w-24 animate-pulse rounded bg-muted" />
            ...
            <div className="space-y-6">
              <section className="rounded-[1.5rem] border border-border bg-white/94 p-5 shadow-[0_20px_55px_-42px_rgba(15,23,42,0.38)] backdrop-blur-sm">
                <div className="mb-4 space-y-2">
                  ...
                  <div className="h-4 w-full max-w-[34rem] animate-pulse rounded bg-muted" />
                </div>
                    ...
                    >
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-2 h-4 w-full max-w-[28rem] animate-pulse rounded bg-muted" />
                          <div className="mt-4 grid gap-2 md:grid-cols-3">
                            {Array.from({ length: 3 }).map((__, cardIndex) => (
                              <div key={cardIndex} className="rounded-xl bg-muted px-3 py-3">
              ...
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
                {Array.from({ length: 2 }).map((_, index) => (
                  <section
                    key={index}
                    className="rounded-[1.5rem] border border-border bg-white/94 p-5 shadow-[0_20px_55px_-42px_rgba(15,23,42,0.38)] backdrop-blur-sm"
                  >
