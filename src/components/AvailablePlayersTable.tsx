                  ...
                  >
                    ...
                    {viewMode === 'detailed' && (
                      <td className="px-4 py-3 whitespace-nowrap">
                        {player.ranking?.rank ? (
                          <div className="flex items-center gap-2">
                            <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${getRankBadge(player.ranking.rank)}`}>
                              {player.ranking.rank}
                            </span>
                            {player.ranking.rank === 1 && <TrophyIcon className="w-4 h-4 text-amber-500" />}
                            {player.ranking.rank <= 10 && player.ranking.rank > 1 && <BoltIcon className="w-4 h-4 text-blue-500" />}
                          </div>
                        ) : (
        ...
        <div className="bg-gray-50 px-4 py-3 border-t border-gray-200">
          <div className="flex flex-col sm:flex-row sm:justify-between gap-2 text-xs text-gray-500">
            ...
            <div className="flex items-center gap-4">
              <span>
                Ranked: {filteredPlayers.filter(p => p.ranking?.rank).length}
              </span>
              <span>
                Watchlisted: {filteredPlayers.filter(p => p.isWatched).length}
              </span>
              <span>
                Drafted: {enhancedPlayers.filter(p => p.isDrafted).length}
              </span>
            </div>
          </div>
        </div>
