  ...
  try {
    ...
    if (resolved?.__STATLY_DC_PLACEHOLDER__) {
      return {
        ok: false,
        error:
          'Data Connect is not configured. This feature is optional and the app will use standard Firestore queries instead.',
      };
    }
    ...
    if (typeof fn !== 'function') {
      return {
        ok: false,
        error:
          'Data Connect SDK is present but the operation is not available. This feature will fall back to standard queries.',
      };
    }
    const res = await fn();
