...
> {
  ...
  const docs = await db.getAll(...docIds.map((id) => db.collection('player_season_stats').doc(id)));
  ...
  for (const doc of docs) {
    ...
    for (const key in data.totals) {
      ...
      existing.summedTotals[canonicalKey] =
        (existing.summedTotals[canonicalKey] || 0) + (data.totals[canonicalKey] || 0);
      existing.totals[canonicalKey] =
        (existing.totals[canonicalKey] || 0) + (data.totals[canonicalKey] || 0);
    }
  ...
  for (const [playerId, data] of aggregated.entries()) {
    ...
    for (const key in data.summedTotals) {
      const canonicalKey = key as CanonicalStatKey;
      stats[canonicalKey] =
        data.totalGames > 0 ? data.summedTotals[canonicalKey] / data.totalGames : 0;
    }
    result.set(playerId, {
      stats,
      totals: data.totals,
