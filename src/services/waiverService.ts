...
class WaiverService {
        ...
        try {
          ...
          if (processResult.request.status === 'APPROVED') {
            ...
            if (config.system === 'ROLLING_LIST' && config.prioritySettings.movesToBack) {
              const updatedPriority = await this.moveUserToBackOfQueue(
                leagueId,
                request.userId,
                priorities
              );
  ...
  private async updateWaiverPriority(leagueId: string, priority: WaiverPriority): Promise<void> {
    ...
    console.log('Waiver priority updated', {
      userId: priority.userId,
      newPriority: priority.currentPriority,
    });
  }

  private async getWaiverRequest(
    _leagueId: string,
    _requestId: string
