    ...
    if (dimension.name === 'operation')
      return {
        ...
        values: dimension.values.filter(
          (value) =>
            sourceRights.content.operations[value as keyof typeof rightsContent.operations] ===
            'allowed'
        ),
      };
...
describe('retained 2020 private derived-use successor', () => {
  it('binds one capture and only its documented fields without changing acquisition provenance', () => {
    ...
    expect(next.proposal.content.scope.dimensions).toContainEqual({
      name: 'retained_method_use',
      values: [original.methodUseCandidate.methodUseId],
    });
    expect(
      next.sourceRights.content.fields
        .filter((field) => field.uses.derived_feature === 'allowed')
        .map((field) => field.sourceField)
    ).toEqual(expect.arrayContaining(['away_points', 'home_points']));
    expect(
      next.sourceRights.content.fields.filter((field) => field.uses.derived_feature === 'allowed')
    ).toHaveLength(2);
  ...
  it('refuses another capture, altered method bytes and out-of-scope fields', () => {
    ...
    expect(() =>
      createRetainedFitzRoyDerivedUseSuccessor({
        ...original,
        methodUseCandidate: {
          ...original.methodUseCandidate,
          content: {
            ...original.methodUseCandidate.content,
            hpnPavMethodId: `hpn-pav-method:${'e'.repeat(64)}`,
          },
        },
      })
    ).toThrow();
    ...
    expect(() =>
      createRetainedFitzRoyDerivedUseSuccessor({
        ...original,
        methodUseCandidate: {
          methodUseId: address('cameron-2020-hpn-method-use', content),
          content,
        },
      })
    ).toThrow();
