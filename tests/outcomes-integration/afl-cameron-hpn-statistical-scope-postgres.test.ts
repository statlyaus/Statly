...
import { createAflTradeHpnFieldMapCandidate, listAflTradeHpnCandidateSourceFields } from '@/server/aflTradeIntelligence/modeling/hpnFieldMapCandidate';
...
import { createAflTradeHpnFieldMapReviewDecision, createAflTradeHpnProjectedFieldMap } from '@/server/aflTradeIntelligence/modeling/hpnProjectedFieldMap';
...
it.each([
...
])('authenticates only the exact Cameron statistical season: $year / $scope', async ({ year, scope, expected }) => {
  const schema = `afl_cameron_scope_${process.pid}_${year}_${scope.includes('2020') ? 'mismatch' : 'exact'}`;
  const scoped = new URL(databaseUrl);
  scoped.searchParams.set('schema', schema);
  const pool = new Pool({ connectionString: databaseUrl, options: `-c search_path=${schema}`, max: 3 });
  ...
  try {
    ...
    await admin.query(`DO $$ BEGIN
      ...
      END IF;
    END $$`);
    await admin.query(`GRANT USAGE ON SCHEMA "${schema}" TO afl_trade_nonproduction_governance_registry_writer`);
    ...
    await ledger.appendBatch({ expectedRevision: (await ledger.load()).revision, records: [{
    ...
    const normalization = (await client.query<{ finalized_at: Date; staging_sha256: string }>(
    ...
    const candidate = createAflTradeHpnFieldMapCandidate({
      environment: 'non_production', competition: 'AFLM', provider: 'footywire',
      capabilityId: map.capabilityId, sourceSchemaSha256: map.sourceSchemaSha256,
      inputKind: 'completed_match_result', validFromSeason: year, validThroughSeason: year,
      providerDecodeMap: map, providerDecodeMapArtifact: mapArtifact, createdAt: at,
      semanticBindings: [
    ...
    const assessment = assessAflTradeHpnSourceFirstCalculationSourceUse({
      ...
      rightsArtifact: createAflTradeCanonicalJsonArtifactRef(authority.sourceRights, authority.sourceRights.content.proposedAt),
      competition: 'AFLM', seasonYear: year, valuationScopeKey: scope, evaluatedAt: at,
      ...
      source: {
        ...
        providerDecodeMapId: map.mapId, providerDecodeMapSha256: mapArtifact.contentSha256,
    ...
    const decision = createAflTradeHpnFieldMapReviewDecision({
      candidate, candidateArtifact, sourceUseAssessment: assessment,
      sourceUseAssessmentArtifact: assessmentArtifact, decision: 'approved',
      reviewerId: 'synthetic-scope-reviewer', rationale: 'Synthetic scope boundary regression.', decidedAt: at,
    });
    const decisionArtifact = createAflTradeCanonicalJsonArtifactRef(decision, at);
    const projected = createAflTradeHpnProjectedFieldMap({ candidate, candidateArtifact, decision, decisionArtifact });
    await new PostgresAflTradeHpnProjectedFieldMapAuthority(client).registerApprovedProjection({
      candidate, candidateArtifact, sourceUseAssessment: assessment,
      sourceUseAssessmentArtifact: assessmentArtifact, reviewDecision: decision,
      decisionArtifact, projectedFieldMap: projected,
    });
    const exact = async (captureId: string, runId: string) => client.transaction(async (transaction) => {
