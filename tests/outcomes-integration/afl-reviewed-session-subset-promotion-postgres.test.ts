...
const schema = `reviewed_subset_${process.pid}_${Date.now()}`;
...
it('promotes one reviewed selection from complete retained inventory with exact replay', async () => {
  const draft = await createRetainedExternalCaptureFixture(sql, false, 'test_fixture', 71);
  ...
  const plan = createAflTradeRetainedExternalCapturePlan({
    ...
    targets: [draft.target, trade.target].sort((a, b) => a.captureId.localeCompare(b.captureId)),
  });
  ...
  const selected = original.content.draftSelections.find((s) => s.selectionNumber === 71)!;
    ...
    records: [
      {
        ...
        retainedSourceLabel: 'Pick 71',
        acceptedTradeTimePick: 71,
        originalClubId: null,
        movements: [
          {
            transferId: transfer.transferId,
  ...
  expect((await reviews.loadCandidate(candidate.candidateId)).content.pickCustody).toHaveLength(1);
