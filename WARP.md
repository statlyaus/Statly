...
# Pre-push validation (CI-aligned: typecheck, lint, Firebase env file, guards, tests, format)
npm run prepush
# Git hooks: configured automatically on npm install (`prepare`); re-run with:
npm run setup:hooks
```
...
- **player*match_stats/{matchUid}*{playerUid}**: Real-time player statistics during matches
