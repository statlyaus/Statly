...
try {
  ...
  execSync('npx tsc src/lib/socketioConfig.ts --outDir temp --target es2020 --module commonjs --esModuleInterop --skipLibCheck', { stdio: 'inherit' });
  
  // Load the compiled JavaScript
  console.log('📦 Loading compiled configuration...');
  const { socketIOConfig } = require('./temp/socketioConfig.js');
  ...
  console.log('Client config types:', {
    ...
    transports: Array.isArray(socketIOConfig.client.transports) ? 'array' : typeof socketIOConfig.client.transports,
    healthCheckIntervalMs: typeof socketIOConfig.client.healthCheckIntervalMs
  });
  
  // Test runtime validation
  console.log('🔍 Testing runtime validation...');
  const { validateSocketIOConfig } = require('./temp/socketioConfig.js');
