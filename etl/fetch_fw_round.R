...
outfile <- Sys.getenv("OUTFILE", unset = ifelse(length(args)>=3, args[[3]], "player_stats_footywire.json"))

# Pull latest for a season/round; if round is NA, fitzRoy returns latest round
df <- fetch_player_stats(season = if (!is.na(season)) season else NULL,
...
rename_if_exists <- function(d, from, to) { if (from %in% names(d)) d <- d %>% rename(!!to := all_of(from)); d }
df <- df %>%
...
keep <- c("season","round","team","opposition","player_name",
