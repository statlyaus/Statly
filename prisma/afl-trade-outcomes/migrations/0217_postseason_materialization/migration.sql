...
-- The caller also authenticates observation/release/review/spell/calculation authority.
-- Complete-case inserts must also pass the independent observation authority guard.
CREATE FUNCTION outcome_postseason_valuation_case_exact(c JSONB)
RETURNS BOOLEAN LANGUAGE plpgsql AS $$
...
BEGIN
 ...
 FOR asset IN SELECT * FROM outcome_event_asset WHERE event_version_id=r->>'eventVersionId' AND status='approved' LOOP
   ...
   IF NOT COALESCE(root->>'assetType'=expected_kind AND component->>'assetKind'=expected_kind
     ...
     AND (expected_kind<>'unsupported_consideration' OR component->>'status'='excluded')
     AND NOT (expected_kind='future_pick_entitlement' AND component->>'status'='supported'),FALSE) THEN RETURN FALSE; END IF;
 END LOOP;
...
BEGIN
 ...
 IF NOT FOUND OR NOT COALESCE(NEW.manifest_json=jsonb_build_object('manifestId',NEW.materialization_manifest_id,'content',c)
   ...
   AND ((c#>>'{request,kind}'='observation' AND c->'valuationCase'='null'::JSONB
     ...
     OR (c#>>'{request,kind}'='complete_trade' AND outcome_postseason_valuation_case_exact(c)))
...
DO $$ DECLARE signature TEXT; BEGIN
 FOREACH signature IN ARRAY ARRAY['outcome_postseason_address(text,jsonb)',
   ...
   'outcome_postseason_valuation_case_exact(jsonb)',
