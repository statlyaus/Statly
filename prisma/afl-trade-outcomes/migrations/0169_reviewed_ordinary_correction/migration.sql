-- Preserve the original scope validator; corrections must validate their immutable scope first.
ALTER FUNCTION validate_outcome_reviewed_admission_scope(JSONB) RENAME TO validate_outcome_reviewed_admission_scope_v1;
CREATE FUNCTION validate_outcome_reviewed_admission_scope(document JSONB)
RETURNS VOID LANGUAGE plpgsql AS $$
DECLARE
 c JSONB:=document->'content'; correction JSONB:=c->'reviewedCorrection'; parent_document JSONB;
 ...
 pick JSONB; draft_year JSONB; draft_type JSONB; recorded_pick JSONB; prior_id JSONB; active_ids TEXT[];
BEGIN
 IF correction IS NULL THEN PERFORM validate_outcome_reviewed_admission_scope_v1(document); RETURN; END IF;
 IF correction->>'schemaVersion' IS DISTINCT FROM 'afl-trade-reviewed-ordinary-correction/v1'
 OR c->'reviewedScope' IS NULL OR c->>'environment'='production'
 OR correction->'registrationId' IS DISTINCT FROM c->'reviewedScope'->'registrationId'
 THEN RAISE EXCEPTION 'Reviewed correction requires its private registered scope'; END IF;
 SELECT candidate_json INTO parent_document FROM outcome_external_reconciliation_candidate
 WHERE candidate_id=correction->>'scopeCandidateId' AND status='finalized';
 IF parent_document IS NULL OR parent_document->'content' ? 'reviewedCorrection'
 OR NOT outcome_external_candidate_retained_sources_current(correction->>'scopeCandidateId',clock_timestamp())
 THEN RAISE EXCEPTION 'Reviewed correction requires its current finalized pre-correction scope'; END IF;
 PERFORM validate_outcome_reviewed_admission_scope_v1(parent_document);
 ...
 IF c-ARRAY['transfers','draftSelections','pickCustody','pickLineage','issues','reviewedScope','reviewedCorrection']
 IS DISTINCT FROM p-ARRAY['transfers','draftSelections','pickCustody','pickLineage','issues','reviewedScope']
 ...
 -- Connected histories containing a special right stay with the special-right owner, including ordinary downstream legs.
 WITH RECURSIVE nodes AS (
  SELECT registered_record->>'transferId' AS id,m,
  ...
  FROM jsonb_array_elements(records) registered_record CROSS JOIN LATERAL jsonb_array_elements(registered_record->'movements') m
 ), links AS (SELECT DISTINCT a.id,b.id AS other FROM nodes a JOIN nodes b USING(key)), blocked(id) AS (
  SELECT registered_record->>'transferId' FROM jsonb_array_elements(records) registered_record JOIN jsonb_array_elements(p->'transfers') t ON t->'transferId'=registered_record->'transferId'
  WHERE t->'asset'->>'kind'<>'pick_entitlement' OR registered_record->'endpoint'->>'kind'='rookie_elevation'
  UNION SELECT l.other FROM blocked b JOIN links l ON l.id=b.id
 ) SELECT array_agg(registered_record->>'transferId' ORDER BY registered_record->>'transferId') INTO eligible FROM jsonb_array_elements(records) registered_record WHERE registered_record->>'transferId' NOT IN (SELECT id FROM blocked);
 SELECT array_agg(b->>'transferId' ORDER BY b->>'transferId') INTO supplied FROM jsonb_array_elements(correction->'bindings') b;
 IF supplied IS DISTINCT FROM eligible THEN RAISE EXCEPTION 'Reviewed correction must cover exactly the eligible ordinary histories'; END IF;
 ...
 OR jsonb_array_length(c->'pickLineage')<>cardinality(eligible)
 ...
 END LOOP;
 IF (SELECT count(DISTINCT v) FROM jsonb_array_elements(custody_evidence) v)<>jsonb_array_length(c->'pickCustody')
 ...
 END LOOP;
 FOR selected IN SELECT value FROM jsonb_array_elements(p->'draftSelections') LOOP
  expected:=selected;
  IF EXISTS(SELECT 1 FROM jsonb_array_elements(c->'pickLineage') l WHERE l->'selectionId'=selected->'selectionId') THEN
   expected:=selected||jsonb_build_object('status',CASE WHEN selected->>'status'='corroborated' THEN 'corroborated' ELSE 'single_source' END);
  END IF;
  IF NOT EXISTS(SELECT 1 FROM jsonb_array_elements(c->'draftSelections') v WHERE v=expected) THEN RAISE EXCEPTION 'Reviewed correction changed an unrelated selection fact'; END IF;
 END LOOP;
 ...
 -- The unchanged finalization trigger additionally enforces exact source-evidence conservation.
END $$;
