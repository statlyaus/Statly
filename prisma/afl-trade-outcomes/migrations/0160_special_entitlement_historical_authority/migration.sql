-- Current authority is independent of the historical snapshot requested by a release.
-- Follow actual identity replacements even when they occurred after the historical cutoff.
CREATE FUNCTION authenticate_outcome_special_entitlement_current_read(id TEXT)
RETURNS VOID LANGUAGE plpgsql AS $$
DECLARE replacement RECORD; head RECORD; award RECORD;
BEGIN
 IF NOT pg_try_advisory_xact_lock(hashtextextended('special-entitlement-custody:'||id,0)) THEN
   RAISE EXCEPTION 'Special entitlement history changed concurrently; retry the transaction' USING ERRCODE='40001';
 END IF;
 SELECT * INTO replacement FROM outcome_special_entitlement_identity_replacement WHERE retired_entitlement_id=id;
 IF FOUND THEN
   PERFORM authenticate_outcome_special_identity_replacement_review(replacement.replacement_json,replacement.approval_decision_id);
   PERFORM authenticate_outcome_special_entitlement_current_read(replacement.replacement_entitlement_id);
   RETURN;
 END IF;
 SELECT * INTO head FROM outcome_special_entitlement_revision WHERE entitlement_id=id ORDER BY revision DESC LIMIT 1;
 IF FOUND AND head.revision>1 THEN
   PERFORM authenticate_outcome_special_entitlement_revision(head.revision_json,head.approval_decision_id);
 ELSE
   SELECT * INTO award FROM outcome_special_entitlement_award WHERE entitlement_id=id;
   IF NOT FOUND THEN RAISE EXCEPTION 'Current special entitlement award is unavailable'; END IF;
   PERFORM authenticate_outcome_special_entitlement_award(award.award_json,award.approval_decision_id);
 END IF;
END $$;

-- Preserve review cutoffs independently of current authority.
CREATE OR REPLACE FUNCTION read_outcome_special_entitlement_same_identity_for_asset(id TEXT, asset_id TEXT, cutoff TIMESTAMPTZ, release_capture_ids TEXT[])
...
BEGIN
  PERFORM authenticate_outcome_special_entitlement_current_read(id);
  ...
  IF NOT FOUND OR selected.revision=1 THEN
    SELECT jsonb_build_object('entitlementId',award.entitlement_id,'award',award.award_json,
      ...
      JOIN outcome_review_decision award_review ON award_review.decision_id=award.approval_decision_id
      WHERE award.entitlement_id=id AND asset.asset_version_id=asset_id AND award_review.decided_at<=cutoff;
  ...
  IF NOT FOUND THEN
    -- The live head was authenticated above; cutoff-selected approvals are historical provenance.
    ...
    SELECT element.edge_json INTO edge FROM jsonb_array_elements(state->'custody') AS element(edge_json) WHERE element.edge_json->>'assetVersionId'=asset_id;
  END IF;
  -- Both cutoff-selected snapshots must have retained release evidence; live-head evidence is not historical output.
