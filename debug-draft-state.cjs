...
async function debugDraftState() {
  try {
    ...
    draft.orders.forEach((order, index) => {
      console.log(`  ${index + 1}. Slot ${order.slot}: ${order.member.displayName} (ID: ${order.member.id}, UserID: ${order.member.userId})`);
    });
    ...
    console.log(`  Current Drafter: ${currentDrafter ? currentDrafter.member.displayName : 'Unknown'} (ID: ${currentDrafter ? currentDrafter.member.id : 'N/A'})`);
    
    console.log('\n🔍 Recent Picks:');
    const recentPicks = draft.picks.slice(-3);
    recentPicks.forEach(pick => {
      console.log(`  Pick #${pick.overall}: ${pick.member.displayName} drafted ${pick.player.name}`);
    });
