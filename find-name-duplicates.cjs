...
async function findPlayerNameDuplicates() {
  ...
  duplicateVariations.forEach(([baseName, variations], index) => {
    ...
    const totalRecords = Array.from(variations.values()).reduce((sum, records) => sum + records.length, 0);
    ...
    variationNames.forEach(varName => {
      ...
      console.log(`       Rounds: ${rounds.slice(0, 5).join(', ')}${rounds.length > 5 ? '...' : ''}`);
      console.log(`       Teams: ${teams.join(', ')}`);
      
      // Show a sample record for context
      if (records.length > 0) {
        const sample = records[0];
        console.log(`       Sample: R${sample.round} - ${sample.disposals} disposals, ${sample.supercoach_score} SC`);
      }
    });
  });
  ...
  duplicateVariations.forEach(([baseName, variations]) => {
    ...
    const overlappingRounds = Array.from(allRounds.entries())
      .filter(([round, varNames]) => varNames.length > 1);
  ...
  console.log(`4. Estimated cleanup: Remove ~${Math.floor(totalDuplicateRecords * 0.3)} redundant records`);
  ...
  if (duplicateVariations.length > 0) {
    ...
    duplicateVariations.slice(0, 10).forEach(([baseName, variations], index) => {
      ...
      console.log(`${index + 1}. "${baseName}" - ${variationCount} variations, ${totalRecords} records`);
    });
  }
}

findPlayerNameDuplicates().catch(console.error).finally(() => process.exit(0));
