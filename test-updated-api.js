...
const base64Json = process.env.FIREBASE_SERVICE_ACCOUNT_JSON_BASE64 || process.env.GOOGLE_SERVICE_ACCOUNT;
if (!base64Json) {
...
async function testAPILogic() {
  try {
    ...
    Object.entries(categories).forEach(([key, value]) => {
      const replacement = key === 'inside50s' ? ' (was clearances)' : 
                         key === 'effectiveDisposals' ? ' (was one percenters)' : 
                         key === 'scoreInvolvements' ? ' (was goal assists)' : '';
      console.log(`   ${key}${replacement}: ${value}`);
    });
    
    console.log('\n📈 Data Availability Check:');
    const availableCount = Object.values(categories).filter(val => val !== undefined && val !== null).length;
    ...
    console.log('   Effective Disposals:', data.stats?.effective_disposals || data.raw_row?.effective_disposals || 'Missing');
    console.log('   Score Involvements:', data.stats?.score_involvements || data.raw_row?.score_involvements || 'Missing');
