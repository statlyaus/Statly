...
async function testUpdatedNineCategory() {
  try {
    ...
    if (snapshot.size > 0) {
      ...
      console.log(
        `8. Effective Disposals (was One Percenters): ${updatedCategories.effectiveDisposals}`
      );
      console.log(
        `9. Score Involvements (was Goal Assists): ${updatedCategories.scoreInvolvements}`
      );
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      ...
      console.log(
        `🎯 All 9 categories populated: ${categoryValues.every((val) => val !== undefined) ? '✅ YES' : '❌ NO'}`
      );
      console.log(
        `📈 Data Completeness: ${((categoryValues.filter((val) => val > 0).length / 9) * 100).toFixed(1)}%`
      );
      ...
      Object.entries(updatedCategories).forEach(([key, value]) => {
        ...
        console.log(
          `  ${meta.abbr.padEnd(4)} | ${meta.label.padEnd(20)} | ${String(value).padStart(2)} | Weight: ${meta.weight}`
        );
      });
