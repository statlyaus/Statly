...
async function checkSpecificDraft() {
  try {
    ...
    draft.orders.forEach((order) => {
      console.log(
        `  Slot ${order.slot}: ${order.member.user.name} (${order.member.isCpu ? 'CPU' : 'Human'})`
      );
    });
    ...
    recentPicks.forEach((pick) => {
      console.log(
        `  Pick ${pick.overall}: ${pick.player.name} - ${pick.member.user.name} (${pick.auto ? 'AUTO' : 'MANUAL'})`
      );
    });
    ...
    console.log(
      `  Player: ${currentOrder?.member.user.name} (${currentOrder?.member.isCpu ? 'CPU' : 'Human'})`
    );
