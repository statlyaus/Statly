...
async function finalSummary() {
  ...
  console.log(`   Unique players: ${new Set(snapshot.docs.map(doc => doc.data().player_name)).size}`);
  console.log(`   Document IDs: ${sampleDoc.id.split('_').length === 3 ? 'Properly formatted' : 'Issue detected'}`);
...
finalSummary().catch(console.error).finally(() => process.exit(0));
