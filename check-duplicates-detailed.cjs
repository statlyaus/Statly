...
async function checkDuplicates() {
  ...
  const logicalDuplicates = Array.from(duplicatesByPlayerRound.entries()).filter(
  ...
  if (logicalDuplicates.length > 0) {
    ...
    logicalDuplicates.slice(0, 10).forEach(([key, records]) => {
      ...
      records.forEach((record) => {
        console.log(
          `     - ${record.docId} (${record.disposals} disposals, ${record.supercoach_score} SC)`
        );
      });
    });
  ...
  const docIdDuplicates = Array.from(duplicatesByDocId.entries()).filter(
    ([docId, players]) => players.length > 1
  );
  ...
  const contentDuplicates = Array.from(duplicatesByContent.entries()).filter(
    ([content, docIds]) => docIds.length > 1
  );
  ...
  if (maxRecords > 25) {
    // More than 25 rounds is unusual
  ...
  console.log(`✅ Total records: ${snapshot.size}`);
  console.log(
    `${logicalDuplicates.length === 0 ? '✅' : '❌'} Logical duplicates: ${logicalDuplicates.length}`
  );
  console.log(
    `${docIdDuplicates.length === 0 ? '✅' : '❌'} Document ID duplicates: ${docIdDuplicates.length}`
  );
  console.log(
    `${contentDuplicates.length === 0 ? '✅' : '❌'} Identical content duplicates: ${contentDuplicates.length}`
  );
  console.log(`✅ Unique players: ${uniquePlayers}`);
  console.log(
    `✅ Rounds covered: ${sortedRounds.length} (${sortedRounds[0][0]} to ${sortedRounds[sortedRounds.length - 1][0]})`
  );

  if (
    logicalDuplicates.length === 0 &&
    docIdDuplicates.length === 0 &&
