...
**Phase 2 - API Integration:** 4. `src/lib/precomputedStats.ts` - Helper library for batch reads 5. `src/app/api/players/route.ts` - Season parameter + pre-computed lookups 6. `src/app/api/leagues/[id]/roster/[userId]/route.ts` - Pre-computed with fallback

**Phase 3 - UI:** 7. `src/app/players/PlayersPageClient.tsx` - Season selector dropdown + auto-season logic
