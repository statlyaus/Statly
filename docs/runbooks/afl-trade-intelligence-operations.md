...
Migration `0230_cameron_2020_retained_private_source_use` accepts only two exact 2020 AFL Tables
captures and their exact field sets when a current, at-most-30-day Gate 0A successor binds the
original capture, rights and decision to the retained HPN method and owner approval hash. The
...
Each capture carries origin gate `gate-decision:612d36de…594bcd6` and origin rights
`source-rights:07808766…5e62a629` (the 18-field capture carries its own documented pair). The
successor is the immediate `version + 1` successor of that origin decision, with no later
successor, and must carry scope dimensions `retained_source_capture`, `original_gate_decision`,
`original_source_rights_artifact`, `retained_method_use`
(`cameron-2020-hpn-method-use:551baef6…93363053`), `hpn_pav_method`
(`hpn-pav-method:a8cef6b1…3a209b43`) and `owner_approval_sha256`
(`ea7eb5fa2ffce3637f45b9e5808bf35b961147b08b6ed302ff96f4d4d72d4210`, which must also appear as
`artifact:` evidence in the successor rights). Original acquisition manifests stay immutable.
Expiry, withdrawal, altered fields, another capture or a changed Gate chain fails closed. The
successor decision and proposal must be content-addressed; a hand-written ledger row with
arbitrary identifiers is rejected.
