...
async function uploadData() {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      const docId = `${data.season}-R${data.round}-${getTeamAbbr(data.team)}-${getTeamAbbr(data.opposition)}_ply_${data.player_name.toLowerCase().replace(/[^a-z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '')}`;
...
function getTeamAbbr(teamName) {
  const abbr = {
    ...
    'Geelong': 'GEE',
    'Gold Coast': 'GCS',
    'GWS': 'GWS', 'Greater Western Sydney': 'GWS',
    'Hawthorn': 'HAW',
    'Melbourne': 'MEL',
    'North Melbourne': 'NTH',
    'Port Adelaide': 'PTA',
